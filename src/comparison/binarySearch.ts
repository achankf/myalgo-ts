...
function findLeft<T, U>(
    ...
    while (true) {
        ...
        const [isFound, at, low] = binsearchHelper(arr, target, toKey, cmp, prevLow, rightMost);
        if (!isFound) {
            return prev;
        }
...
function findRight<T, U>(
    ...
    while (true) {
        ...
        const [isFound, at, , high] = binsearchHelper(arr, target, toKey, cmp, leftMost, prevHigh);
        if (!isFound) {
            return prev;
        }
...
export function lowerBound<T, U>(
    ...
    {
        const [isFound, ret, low] = binsearchHelper(arr, target, toKey, cmp, 0, arr.length - 1);
        if (!isFound) {
            return ret;
        }
...
export function upperBound<T, U>(
    ...
    {
        const [isFound, ret, , high] = binsearchHelper(arr, target, toKey, cmp, 0, arr.length - 1);
        if (!isFound) {
            return ret;
        }
...
export function binarySearch<T, U>(
    ...
    {
        const [isFound, ret, low, high] = binsearchHelper(arr, target, toKey, cmp, 0, arr.length - 1);
        if (!isFound) {
            return undefined;
        }
