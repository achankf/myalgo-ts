...
export function cmpList<A, B>(cmp: (a: A, b: B) => number, as: A[], bs: B[]) {
  const [cmpResult] = cmpListHelper(cmp, as, bs);
  return cmpResult;
}

export function listEqual<A, B>(cmp: (a: A, b: B) => number, as: A[], bs: B[]) {
  return cmpList(cmp, as, bs) === 0;
}

/** Find the first index where as and bs diverge. */
export function listDiff<A, B>(cmp: (a: A, b: B) => number, as: A[], bs: B[]) {
