...
test("base case (simple reachable)", () => {
  ...
  const { parent: parent1 } = assertDefined(aStarAdjList(graph, () => 1, 0, 1));

  expect(
    listEqual((a, b) => a - b, extractPath(parent1, 0, 1) || [], [1, 0])
  ).toBeTruthy();

  const { parent: parent2 } = assertDefined(aStarAdjList(graph, () => 1, 1, 0));

  expect(
    listEqual((a, b) => a - b, extractPath(parent2, 1, 0) || [], [0, 1])
  ).toBeTruthy();
});
...
test("base case (same source-destination)", () => {
  ...
  const sourceDest = 1;
  expect(() => aStarAdjList(graph, () => 1, sourceDest, sourceDest)).toThrow();
});
...
test("uniform cost", () => {
  ...
  const { distance, parent } = assertDefined(aStarAdjList(graph, () => 1, 0));

  expect(
    listEqual((a, b) => a - b, extractPath(parent, 0, 5) ?? [], [5, 0])
  ).toBeTruthy();
  expect(distance.get(0)).toBe(0);
  expect(distance.get(5)).toBe(1);

  expect(
    listEqual((a, b) => a - b, extractPath(parent, 0, 4) || [], [4, 1, 0]) ||
      listEqual((a, b) => a - b, extractPath(parent, 0, 4) || [], [4, 2, 0])
  ).toBeTruthy();
...
test("Dijkstra", () => {
  ...
  const { distance, parent } = assertDefined(aStarAdjList(graph, weight, 0));

  expect(
    listEqual((a, b) => a - b, extractPath(parent, 0, 5) || [], [5, 3, 0])
  ).toBeTruthy();
  ...
  expect(
    listEqual((a, b) => a - b, extractPath(parent, 0, 3) || [], [3, 0])
  ).toBeTruthy();
  expect(distance.get(3)).toBe(4);

  expect(
    listEqual((a, b) => a - b, extractPath(parent, 0, 1) || [], [1, 2, 0])
  ).toBeTruthy();
  expect(distance.get(1)).toBe(3);

  const { parent: parent2 } = assertDefined(aStarAdjList(graph, weight, 4));
  ...
  const { parent: parent3 } = assertDefined(
    aStarAdjList<number>(graph, weight, 6)
  );
  ...
  expect(
    listEqual((a, b) => a - b, extractPath(parent3, 6, 7) || [], [7, 6])
  ).toBeTruthy();
});
