 ...
 */
export interface MyIterator<T> extends Iterable<T> {
  /**
   ...
   */
  filter: (pred: (t: T, i: number) => boolean) => MyIterator<T>;
   ...
   */
  take: (count: number) => MyIterator<T>;
...
function forEach<T>(
  it: Iterable<T>,
  callBackFn: (t: T, i: number) => boolean | undefined
) {
  let i = 0;
  for (const t of it) {
    if (!callBackFn(t, i)) {
 ...
 */
export function toIt<T>(it: Iterable<T>): MyIterator<T> {
  return {
    ...
    forEach: (callBackFn: (t: T, i: number) => boolean | undefined) =>
      forEach(it, callBackFn),
    inject: () => inject(it),
    map: <U>(mapper: (t: T, i: number) => U) => toIt(map(it, mapper)),
    pin: <U>(hammer: (prev: T, cur: T) => U) => toIt(pin(it, hammer)),
    reduce: <U>(reducer: (acc: U, cur: T, i: number) => U, base: U) =>
      reduce(it, reducer, base),
 ...
 */
export abstract class MyIterable<T> implements MyIterator<T> {
  ...
  public reduce = <U>(reducer: (acc: U, cur: T, i: number) => U, base: U) =>
    reduce(this, reducer, base);

  public some = (pred: (t: T, i: number) => boolean) => some(this, pred);

  public every = (pred: (t: T, i: number) => boolean) => every(this, pred);

  public filter = (pred: (t: T, i: number) => boolean) =>
    toIt(filter(this, pred));

  public forEach = (callBackFn: (t: T, i: number) => boolean | undefined) =>
 ...
 */
export function genBy<T>(generator: (i: number) => T): MyIterator<T> {
...
export function repeat<T>(t: T): MyIterator<T> {
 ...
 */
export function combine<T>(...its: Array<Iterable<T>>): MyIterator<T> {
