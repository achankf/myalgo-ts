 ...
 */
export interface IMap<K, V> extends Iterable<[K, V]>, IMapReadonly<K, V> {
...
export function fullOuterJoin<T, U, V>(
  ...
  return combine(...maps.map((map) => map.keys())).reduce((acc, key) => {
    const v = combinator(
      ...maps
        .map((map) => assertDefined(map.get(key)))
