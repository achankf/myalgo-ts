 ...
 */
export function negate<T>(map: IMapReadonly<T, number>) {
...
export function average(arg1: number, ...args: number[]) {
 ...
 */
export function sumBy<T>(mapper: (t: T) => number, it: Iterable<T>) {
    let ret = 0;
    for (const t of it) {
        ret += mapper(t);
    }
 ...
 */
export function sumMaps<T>(...maps: Array<IMapReadonly<T, number>>) {
