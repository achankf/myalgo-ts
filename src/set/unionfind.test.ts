...
test("iterator", () => {
  ...
  // test map()
  const totalSize = set.map((ts) => ts.size).reduce((acc, x) => acc + x, 0);
...
test("get", () => {
  ...
  expect(
    listEqual((x, y) => x - y, Array.from(set.get(1)).sort(), a)
  ).toBeTruthy();

  expect(
    listEqual((x, y) => x - y, Array.from(set.get(11)).sort(), b)
  ).toBeTruthy();

  expect(
    listEqual((x, y) => x - y, Array.from(set.get(111)).sort(), c)
  ).toBeTruthy();
});
